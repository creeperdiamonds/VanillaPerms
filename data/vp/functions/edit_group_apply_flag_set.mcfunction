# vp:edit_group_apply_flag_set.mcfunction
# Sets up the list-rebuilding process for setting a group's flags.
...
# Start the rebuild loop. This loop is now generic and uses the 'action' value from storage.
...
# After editing the group, recalculate flags for all of its members.
# The group name is still in 'storage vp:temp arg' from the initial call.
